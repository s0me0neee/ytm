...
# Releases are cut by `dist` (cargo-dist) from a pushed tag: `v1.0.0` builds both
# binaries for five targets and publishes one GitHub Release. Nothing is released
# by hand — pushing the tag is the whole action.
dist plan --tag=v1.0.0    # what that tag would announce, without building anything
dist build --artifacts=local --target=aarch64-apple-darwin   # one target, here
...
On **MSVC** that directory needs an `mpv.lib`, which the mpv-dev package does not contain:
`rustc-link-lib=mpv` makes link.exe ask for `mpv.lib`, and the `libmpv.dll.a` in the archive
is a GNU import library it cannot read. One is built from the DLL's own export table —
`dumpbin /exports` into a `.def`, then `lib /def: /name:libmpv-2.dll` — which is what the
Windows half of `.github/build-setup.yml` does and what a local MSVC build has to do too.

### Releasing

`dist` (cargo-dist) owns it, configured in `dist-workspace.toml` and generating
`.github/workflows/release.yml` — which is autogenerated and should never be hand-edited;
change the config and run `dist generate`. A pushed tag matching `**[0-9]+.[0-9]+.[0-9]+*` is
the trigger, the version in it has to match the packages' own, and both `yt-music-tui` (the
`ytm` binary) and `ytm-gui` are announced together under it.

Two things the release build needs that no package manager provides, so they live in
`.github/build-setup.yml` — a bare list of steps `dist` splices into the build job. It sits
beside the workflows rather than in them because GitHub parses anything under
`workflows/` as a workflow of its own, and a step list is not one.

- **libmpv on Windows**, per the section above: the mpv-dev archive is fetched at a *pinned*
  dated build (a release should say which libmpv it linked against) and the non-`v3` one,
  since `-v3` targets x86-64-v3 and would fault on older CPUs. The MSVC import library is
  generated there.
- **The GUI's frontend.** `tauri::generate_context!()` embeds `frontendDist` at *compile*
  time, and `beforeBuildCommand` is run by the tauri CLI — which dist is not, it builds with
  plain cargo. So `pnpm build` has to have run before `dist build`, on every runner, or
  `ytm-gui` compiles against a `gui/dist` that does not exist. `--ignore-scripts` on the
  install is deliberate: `react-devtools` drags in electron, whose postinstall downloads
